...
형식: {{"USER_MESSAGE": "사용자에게 보여줄 짧은 안내/질문 한 단락", "CONTRACT_DRAFT": "용역계약서 초안. 수집된 정보와 템플릿을 활용해 핵심 조항만 한국어로 간결히 작성. 미기재 항목은 '미기재'로 표기. 서론/메타설명/기타 문구 없이 계약 조항만"}}
"""
...
출력 규칙 (엄수):
아래 JSON 형식으로만 응답하세요. 다른 텍스트는 절대 금지.
형식: {{"USER_MESSAGE": "사용자에게 보여줄 짧은 안내/질문 한 단락, 새 단계 시작 알림 포함 가능", "CONTRACT_DRAFT": "용역계약서 초안. 수집된 정보와 템플릿을 활용해 핵심 조항만 한국어로 간결히 작성. 미기재 항목은 '미기재'로 표기. 서론/메타설명/기타 문구 없이 계약 조항만"}}
"""
