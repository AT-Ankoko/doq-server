...
- 'client' (의뢰인): {{client_name}}  
- 'provider' (용역자): {{service_provider_name}}

용역 내용, 작업 기간, 대금, 수정 조건 등 하나씩 확인하면서 진행하겠습니다.

먼저 질문드릴게요:

{{client_name}}님(client), 어떤 용역을 의뢰하실 계획인가요?
간단히 작업 내용을 말씀해 주세요. 예: "홈페이지 제작", "마케팅 전략 수립", "번역 작업" 등
"""
...
출력 형식(고정):
USER_MESSAGE: <사용자에게 보여줄 짧은 안내/질문 한 단락>
CONTRACT_DRAFT: <"용역계약서 초안". 수집된 정보와 템플릿을 활용해 핵심 조항만 한국어로 간결히 작성. 미기재 항목은 "미기재"로 표기. 서론/메타설명/기타 문구 없이 계약 조항만>
"""
...
위 정보는 모두 참고용이며, 사용자에게 노출하지 마세요.
출력 형식(고정):
USER_MESSAGE: <사용자에게 보여줄 짧은 안내/질문 한 단락, 새 단계 시작 알림 포함 가능>
CONTRACT_DRAFT: <"용역계약서 초안". 수집된 정보와 템플릿을 활용해 핵심 조항만 한국어로 간결히 작성. 미기재 항목은 "미기재"로 표기. 서론/메타설명/기타 문구 없이 계약 조항만>
"""
