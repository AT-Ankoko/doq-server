...
**"advance": true (다음 단계로 이동) - 매우 엄격한 조건**
필수 조건 (모두 충족해야 함):
1. **양측 발화 확인**: 최근 대화 로그에 client와 provider 양측 모두 발언이 있어야 함
2. **명시적 합의**: 양측 모두 해당 조건에 명시적으로 동의했거나 합의를 표현
3. **순차적 동의**: 한쪽이 조건을 제시 → 상대방이 명확히 수락 ("네", "동의합니다", "알겠습니다", "진행하겠습니다", "합의합니다" 등)

구체적 예시:
- client: "130만원으로 합의하시죠" → provider: "네, 130만원에 진행하겠습니다" ✓ true
- client: "수정 3회로 하죠" → provider: "3회로 합의합니다" ✓ true
- client: "5회까지 가능할까요?" → provider: "4회까지 무료로 해드리겠습니다" → client: "4회로 합의합니다" ✓ true

**"advance": false (현재 단계 유지) - 기본값, 의심스러우면 false**
반드시 false인 경우:
- **한쪽만 발언한 경우**: 대화 로그에 client 또는 provider 중 한쪽만 있음
- **제안만 있고 수락 없음**: 한쪽이 "...하시죠", "...로 할까요?" 제안했으나 상대방 응답 없음
- **유보적 응답**: "검토하겠습니다", "생각해보겠습니다", "고민 중입니다"
- **반대/거절**: "어렵습니다", "힘듭니다", "...은 안 됩니다"
- **추가 협상**: "...로 조정 가능할까요?", "대신 ...는 어떨까요?"
- **정보 부족**: 현재 단계의 핵심 정보가 아직 제공되지 않음
- **질문만 있음**: 사용자가 추가 질문만 하고 동의 표현 없음

[절대 규칙]
- 키워드("합의", "동의")만으로 판단 금지 → 반드시 양측 발화 확인 필수
- 의심스러우면 무조건 false (안전 우선)
- introduction 단계는 예외: 사용자가 의미 있는 입력을 하면 진행 가능
...
현재 단계: {{current_step}}
추출 대상 필드: {{target_field}}

[지침]
1. 양측(client, provider)이 최종적으로 동의한 내용을 추출하세요.
2. 구어체가 아닌, 계약서에 들어갈 수 있는 명확한 문어체로 요약하세요.
3. 합의된 내용이 없다면 null을 반환하세요.
4. 금액은 숫자와 단위(원)를 포함하여 명확히 표기하세요. (예: "월 800,000원")
5. 기간은 구체적인 날짜나 기간을 명시하세요. (예: "2025년 12월 9일부터 3개월간")

[출력 형식]
```json
{
  "extracted_value": "최종 합의된 내용 요약 (문자열)"
}
```
"""
