...
**필수 조건: 반드시 client와 provider 양측 모두 발화했고, 둘 다 동의한 경우만 true**
- 양측(client, provider)이 해당 조건에 모두 명시적으로 동의했거나, 합의에 도달한 경우
- 한쪽이 제안하고 다른 쪽이 긍정적으로 수락한 경우  
  (예: "좋습니다", "네", "동의합니다", "ok", "괜찮습니다", "알겠습니다", "진행하겠습니다")
- 대화 로그에 client와 provider 양측의 발화가 모두 포함되어 있고, 서로의 조건을 수용한 경우
- 사용자가 다음 단계로 넘어가길 명시적으로 요청한 경우  
  (예: "다음으로 넘어가주세요", "패스", "그 정도면 됐어요")

"advance": false (현재 단계 유지) - 기본값
- **한쪽만 조건을 제시했고, 상대방의 응답이나 동의가 아직 없는 경우 (가장 중요)**
- 대화 로그에 client 또는 provider 중 한쪽의 발화만 있는 경우
- 한쪽이 조건을 제시했지만 상대방이 "검토하겠습니다", "생각해보겠습니다" 등 유보적 반응을 보인 경우
...
[중요 주의사항]
- "합의", "동의" 같은 키워드가 있어도, 대화 로그에 양측 발화가 모두 없다면 반드시 false
- 한쪽이 "130만원으로 합의하시죠"라고 제안만 한 경우 → false (상대방 동의 대기)
- 양측이 각각 발언하고 서로 수락했을 때만 → true
