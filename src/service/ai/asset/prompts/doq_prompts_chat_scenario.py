...
1. "advance": true 인 경우 (다음 단계로 이동)
   - **양측(client, provider)이 현재 단계의 조건에 대해 모두 동의하거나 합의가 이루어진 경우**
   - 한쪽의 제안에 대해 다른 쪽이 수락, 동의, 긍정의 표현을 한 경우
     (예: "좋습니다", "네", "동의합니다", "괜찮아요", "ㅇㅇ", "ㄱㅊ", "ok", "오케이", "진행하시죠", "문제 없습니다")

2. "advance": false 인 경우 (현재 단계 유지)
   - **한쪽이 조건을 제시했으나, 아직 상대방의 확인이나 동의를 받지 못한 경우** (즉시 넘어가면 안 됨)
   - 사용자의 응답이 질문의 요지를 피해가거나 불완전한 경우
   - 사용자 또는 상대방이 조건 변경을 요청한 경우
   - 합의하지 않고 반대 또는 거절 의사를 표현한 경우
   - 사용자가 추가 질문을 한 경우
