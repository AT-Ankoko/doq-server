...
SCENARIOS = [
    ...
    },
    {
        ...
        "steps": [
            ...
            ("client", "네, 기획과 콘텐츠는 준비되어 있고 디자인만 진행해주시면 됩니다. 기간은 1개월 예상합니다."),
            ("provider", "페이지 수가 많아 1개월은 빠듯합니다. 최소 6주는 필요합니다."),
            ("client", "일정이 좀 늘어나네요. 그럼 6주로 하시죠. 예산은 500만원입니다."),
            ("provider", "6주 작업에 500만원은 어렵습니다. 디자인 범위가 넓어 800만원은 되어야 합니다."),
            ("client", "예산 차이가 크군요. 그럼 중간인 700만원으로 합의하시죠."),
            ("provider", "네, 700만원이면 진행 가능합니다."),
            ("client", "완료 후 간단 수정은 1년 포함인가요?"),
            ("provider", "1년은 너무 길어서 어렵고, 3개월 정도는 무상으로 지원해드릴 수 있습니다."),
            ("client", "네 알겠습니다. 3개월로 하죠."),
            ("client", "계약서 작성 부탁드립니다."),
        ],
    },
    {
        ...
        "steps": [
            ...
            ("client", "인스타그램과 매장 내 게시용, 두 종류 다 제작하고 싶습니다. 기간은 3개월로 하고 싶습니다."),
            ("provider", "네, 3개월 계약 가능합니다."),
            ("client", "예산은 월 50만원 정도 생각하고 있습니다."),
            ("provider", "두 종류 포스터를 매월 제작하는 데 월 50만원은 많이 적습니다. 최소 월 100만원은 받아야 합니다"),
            ("client", "예산 초과네요. 그럼 포스터 제작 횟수를 줄여서 월 80만원에 맞춰주세요."),
            ("provider", "네, 그럼 월 4회 제작 기준으로 월 80만원에 진행하겠습니다."),
            ("client", "보고서는 매주 주시나요?"),
            ("provider", "네, 매주 월요일에 진행 상황을 요약해 보내드리겠습니다."),
            ("client", "계약서 작성해주세요. 특약 사항은 없습니다."),
        ],
    },
]
...
async def run_scenario(idx: int, scenario: dict):
    ...
    async with websockets.connect(uri) as ws:
        ...
        for step_idx, (role, text) in enumerate(scenario.get("steps", []), start=1):
            print(f"\n--- Step {step_idx} ---")
            await send_message(ws, role, text, display_name_map)
